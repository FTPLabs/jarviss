// HTTP Lua API: GET, POST, JSON requests
  ...
  /// Check whether a Lua table is a pure sequence (array-like: keys 1..=n, no extra keys).
  /// BUG FIX: old version used filter_map to silently drop non-integer keys, causing
  /// mixed tables like {1, 2, name="x"} to be detected as arrays and lose string keys.
  fn is_sequence_table(table: &Table) -> mlua::Result<bool> {
      let seq_len = table.raw_len() as usize;
      if seq_len == 0 {
          return Ok(false);
      }
      // Count ALL key-value pairs (including non-integer keys)
      let total: usize = table.clone()
          .pairs::<Value, Value>()
          .filter_map(|r| r.ok())
          .count();
      // If total == seq_len, every key is a sequential integer (1..=seq_len)
      Ok(total == seq_len)
  }
      ...
      if is_sequence_table(&table)? {
          ...
          for pair in table.pairs::<Value, Value>() {
              ...
              let key = match &k {
                  ...
                  _ => return Err(mlua::Error::runtime("JSON object key must be a string or number")),
              };
